...
alter table public.automation_executions
  add column if not exists recipient_ig_user_id text;

alter table public.automation_executions
  add constraint automation_executions_status_check
  check (status in ('queued', 'skipped', 'succeeded', 'failed', 'awaiting_cta'));

create table if not exists public.automation_cta_sessions (
...
create unique index if not exists uq_automation_cta_sessions_payload
  on public.automation_cta_sessions(payload);

create index if not exists idx_automation_cta_sessions_event
  on public.automation_cta_sessions(event_id, automation_id);

create index if not exists idx_automation_cta_sessions_status
  on public.automation_cta_sessions(status, created_at);

alter table public.automation_cta_sessions enable row level security;
...
begin
  ...
  loop
    ...
    insert into public.automation_actions (automation_id, type, template, use_ai, sort_order, cta_text)
    values (p_automation_id, v_action_type, v_template, v_use_ai, v_sort_order, v_cta_text);
  end loop;
end;
$$;
