...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!connectionId) return errorResponse(400, 'connectionId is required', req)
    if (!isUuid(connectionId)) return errorResponse(400, 'connectionId must be a UUID', req)
    ...
    if (existingError) {
      ...
      return errorResponse(500, 'Unable to list posts', req)
    }
    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        if (upsertError) {
          ...
          return errorResponse(500, 'Unable to store posts', req)
        }
      }
